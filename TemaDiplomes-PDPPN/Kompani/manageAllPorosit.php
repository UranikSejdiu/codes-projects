...
switch ($_POST["action"]) {
    ...
    case "fetchalldata":
        ...
        while ($row = mysqli_fetch_assoc($query)) {
            ...
            $sub_array[] = $row['phone'];
            $sub_array[] = $status;
            $sub_array[] = '<a href="javascript:void(0);" data-id="' . $row['porosiaID'] . '"  class="btn button-success btn-sm editbtnPorosi" >Ndrysho</a>';
            $data[] = $sub_array;
        }
    ...
    case 'statusiData':
        $id = $_POST['id'];
        $sql = "SELECT statusi FROM porosit WHERE porosiaID=$id LIMIT 1";
        $query = mysqli_query($con, $sql);
        $row = mysqli_fetch_assoc($query);
        echo json_encode($row);
        break;
    ...
    case 'statusUpdate':
        ...
        $sql = "UPDATE porosit SET statusi='$status' WHERE porosiaID=$id";
        $query = mysqli_query($con, $sql);
        if ($query == true) {
